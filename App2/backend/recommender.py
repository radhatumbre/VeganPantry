...
class RecipeRecommender:
    ...
    def recommend_recipes(self, user_ingrs):
        ...
        def instructions_with_vegan_available(instructions, ingr_list, user_ingrs):
            # If instructions are a list, process each step
                ...
                for step in instructions:
                    for i in ingr_list:
                        if i in self.non_vegan_ingr:
                            ...
                            for alter in result_list:
                                if alter in user_ingrs:
                                    step = step.replace(i, alter)
                                    found = True
                                    break
                            if not found:
                                step = step.replace(i, result_list[0])
                    new_instructions.append(step)
                return new_instructions
            elif isinstance(instructions, str):
                # Handle instructions as string (if they're not a list)
                for i in ingr_list:
                    if i in self.non_vegan_ingr:
                        filtered_df = self.df_non_vegan_alter[
                            self.df_non_vegan_alter["BaseIngredient"] == i
        ...
        def instructions_mod_list(instructions_list):
            # Only use ast.literal_eval if instructions are in string form
            if isinstance(instructions_list, str):
                try:
                    instructions_list = ast.literal_eval(instructions_list)
                except (ValueError, SyntaxError):
                    return instructions_list  # Return the string if it can't be parsed
            return instructions_list  # Return the original list if it's already a list
