...
class VeganRecipeRecommender:
    def __init__(self, df, mapping_dict, non_vegan_ingr, df_non_vegan_alter):
        ...
        # Apply safe_literal_eval to convert string representations of lists
    ...
    def recommend_recipes(self, user_ingrs):
        ...
        self.df['base_ingredients'] = self.df.apply(lambda row: self.extract_base_ingr(row['ingredients'], row['instructions']), axis=1)
        self.df['instructions'] = self.df.apply(lambda row: self.base_instructions(row['instructions'], row['ingredients']), axis=1)
        ...
        self.df['HasEnoughIngredients'] = self.df['vegan_ingredients'].apply(lambda x: self.check_ingredients(x, user_ingrs))
        suitable_recipes = self.df[self.df['HasEnoughIngredients']]
        
        # Adjust vegan ingredients with available user ingredients
        suitable_recipes['tags'] = suitable_recipes['base_ingredients'].apply(lambda x: self.vegan_with_available(x, user_ingrs))
        
        # Update instructions with vegan alternatives available to the user
        suitable_recipes['instructions'] = suitable_recipes.apply(lambda row: self.instructions_with_vegan_available(row['instructions'], row['base_ingredients'], user_ingrs), axis=1)
        ...
        print("Base ingredients lengths:", self.df['base_ingredients'].apply(len))
        print("Instructions lengths:", self.df['instructions'].apply(len))
        ...
        return recommended_recipes[['name', 'Similarity', 'instructions']]
